...
# openssl
[ ! -d openssl ] && $WGET https://github.com/openssl/openssl/releases/download/openssl-${v_openssl}/openssl-${v_openssl}.tar.gz && tar -xf openssl-${v_openssl}.tar.gz && mv openssl-${v_openssl} openssl && rm openssl-${v_openssl}.tar.gz
...
# libiconv
if [ ! -d libiconv ]; then
    $WGET https://ftp.gnu.org/pub/gnu/libiconv/libiconv-${v_libiconv}.tar.gz
    tar -xf libiconv-${v_libiconv}.tar.gz
    mv libiconv-${v_libiconv} libiconv
    rm libiconv-${v_libiconv}.tar.gz
fi
